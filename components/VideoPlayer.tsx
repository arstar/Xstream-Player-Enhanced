...
export default function VideoPlayer({
    ...
    useEffect(() => {
        ...
        // The live relay endpoint deliberately hides the provider URL (and therefore
        // has no `.m3u8` suffix). It still returns an HLS manifest, so treating it as
        // a regular video URL makes Chromium hand the manifest to its native demuxer
        // and fail with PipelineStatus::DEMUXER_ERROR_COULD_NOT_PARSE.
        const isLiveRelay = /\/api\/relay\?[^#]*\btype=live(?:&|$)/i.test(src);
        const isHLS = src.toLowerCase().includes('.m3u8') || isLiveRelay;
        ...
        const isLiveHls = isProviderLive || isBroadcastRelay || isLiveRelay;
        ...
        const preferHlsJs = (isVodRelay || isLiveRelay) && Hls.isSupported();
