...
/**
 * Optional comma-separated list of category country/language prefixes.
 * Providers commonly name categories as `NL | ...`, `UK | ...`, etc.
 * Leaving the variable empty preserves the upstream behaviour and imports all
 * categories.
 */
const CATEGORY_PREFIXES = new Set(
    (process.env.XSTREAM_CATEGORY_PREFIXES ?? '')
        .split(',')
        .map(prefix => prefix.trim().toUpperCase())
        .filter(Boolean)
);

function isAllowedCategory(categoryName: string): boolean {
    if (CATEGORY_PREFIXES.size === 0) return true;

    // Some providers use a capital I instead of a pipe as the separator.
    const match = categoryName.trim().match(/^([A-Z]+)\s*(?:\||I\b)/i);
    return match !== null && CATEGORY_PREFIXES.has(match[1].toUpperCase());
}
...
export async function syncXtreamToSqlite(
    ...
    for (const step of SYNC_STEPS) {
        ...
        const categories = rawCategories
            ...
            .filter(category => category.category_id && isAllowedCategory(category.category_name));
        const allowedCategoryIds = new Set(categories.map(category => category.category_id));
        ...
        for (let index = 0; index < total; index += SQLITE_BATCH_SIZE) {
            ...
            const batch = rawStreams
                ...
                .filter(stream => (
                    ...
                    && stream.category_id
                    && stream.name
                    && allowedCategoryIds.has(stream.category_id)
                ));

            sqlite.saveStreams(batch);
            keepIds.push(...batch.map(stream => String(stream.id)));
            processed = Math.min(index + SQLITE_BATCH_SIZE, total);
