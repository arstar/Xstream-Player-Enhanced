...
export const useTvNavigation = () => {
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            // Backspace is a browser keyboard editing key, never a navigation
            // command while a field has focus. In particular, when the caret was
            // at the end of a search query the old caret-edge logic let it fall
            // through to `isBackKey`, sending the user to the previous page.
            // Do not preventDefault here: the input must still delete a character.
            if (isFormControl && e.key === 'Backspace') {
                return;
            }
